  ...
  return (
    <div className="flex items-center justify-center gap-2 md:gap-6 p-2 px-5 md:px-16">
      {productionHouseList.map((item) => (
        <div
          key={item.id}
          className="border-[2px] border-gray-600 rounded-lg shadow-black shadow-lg relative hover:scale-110 transition-all duration-200 ease-in-out cursor-pointer"
        >
          <img src={item.image} alt={item.name} className="absolute top-0" />
          <video
            src={item.video}
            alt={item.name}
            autoPlay
